...
import {
  ...
  WalletConfigs,
  PriceProviders,
} from "@acala-network/sdk/wallet/types.d";
...
import { DexPriceProvider } from "@acala-network/sdk/wallet/price-provider/dex-price-provider";
import { AggregateProvider } from "@acala-network/sdk/wallet/price-provider/aggregate-price-provider";
import { AcalaExpandBalanceAdapter } from "@acala-network/sdk/wallet/balance-adapter/types";
import { AcalaBalanceAdapter } from "@acala-network/sdk/wallet/balance-adapter/acala";
import { TokenProvider } from "@acala-network/sdk/wallet/token-provider/type";
import { DIDWeb3Name } from "@acala-network/sdk/wallet/web3name/did";
import { subscribeLiquidCrowdloanTokenPrice } from "@acala-network/sdk/wallet/utils/get-liquid-crowdloan-token-price";
import { AcalaTokenProvider } from "./tokenProvider";

export class AcalaWallet implements BaseSDK {
  private api: AnyApi;
   ...
   */
  public subscribeTokens = memoize(
    (type?: TokenType | TokenType[]): Observable<TokenRecord> => {
   ...
   */
  public subscribeToken = memoize(
    (target: MaybeCurrency): Observable<Token> => {
   ...
   * @param address
   */
  public subscribeBalance = memoize(
    (symbol: MaybeCurrency, address: string): Observable<BalanceData> => {
   ...
   * @param token
   */
  public subscribeIssuance = memoize(
    (symbol: MaybeCurrency): Observable<FN> => {
      const token = this.tokenProvider.getToken(symbol);
    ...
    (
      ...
      return combineLatest({
        ...
        switchMap(({ nativeToken, feeToken }) => {
          ...
          const isFeeToken =
            forceToCurrencyName(feeToken) === forceToCurrencyName(token);
          ...
          return combineLatest({
            ...
            map(({ accountInfo, tokenInfo, feeInfo }) => {
              ...
              const nativeFreeBalance = FN.fromInner(
              ...
              const nativeLockedBalance = FN.fromInner(
                ...
                FN.fromInner(
                  accountInfo.data.feeFrozen.toString(),
                  nativeToken.decimals
                )
              );
              const isDefaultFee =
              ...
              const targetFreeBalance: FixedPointNumber = isNativeToken
                ? nativeFreeBalance
                : tokenInfo.free;
              const targetLockedBalance: FixedPointNumber = isNativeToken
                ? nativeLockedBalance
                : tokenInfo.locked;
              const ed = token.ed;
  ...
  public async getSuggestInput(
    ...
    return firstValueFrom(
      this.subscribeSuggestInput(token, address, isAllowDeath, fee)
    );
  }

  public getPresetTokens(ignoreCheck = false): PresetTokens {
    if (!ignoreCheck && this.isReady$.value === false)
    ...
    const data: PresetTokens = {
      ...
      stableToken:
        tokens[
          forceToCurrencyName(this.api.consts.cdpEngine.getStableCurrencyId)
        ],
      liquidToken:
        tokens[
          forceToCurrencyName(
            this.api.consts.homa?.liquidCurrencyId ||
              this.api.consts.homaLite?.liquidCurrencyId ||
              ""
          )
        ],
      stakingToken:
        tokens[
          forceToCurrencyName(
            this.api.consts.homa?.stakingCurrencyId ||
              this.api.consts.homaLite?.stakingCurrencyId ||
              ""
          )
        ],
    };
   ...
   */
  public subscribePrice = memoize(
    (
      symbol: MaybeCurrency,
      type = PriceProviderType.AGGREGATE
    ): Observable<FN> => {
      ...
      if (isLiquidCrowdloanName(name) && type !== PriceProviderType.DEX) {
        return this.subscribePrice(stakingToken, type).pipe(
          switchMap((price) =>
            subscribeLiquidCrowdloanTokenPrice(
              this.api,
              price,
              getLiquidCrowdloanIdFromName(name)
            )
          )
        );
      }
      ...
      if (
        ChainType.KARURA === getChainType(this.consts.runtimeChain) &&
        name === "sa://0" &&
        stakingToken
      ) {
      ...
      if (
        type === PriceProviderType.MARKET &&
        this.priceProviders[PriceProviderType.MARKET]
      ) {
        return (
          this.priceProviders[PriceProviderType.MARKET]?.subscribe(token) ||
      ...
      if (
        type === PriceProviderType.AGGREGATE &&
        this.priceProviders[PriceProviderType.AGGREGATE]
      ) {
        return (
          this.priceProviders[PriceProviderType.AGGREGATE]?.subscribe(token) ||
          of(FixedPointNumber.ZERO)
        );
      }

      if (
        type === PriceProviderType.DEX &&
        this.priceProviders[PriceProviderType.DEX]
      ) {
        return (
          this.priceProviders[PriceProviderType.DEX]?.subscribe(token) ||
          of(FixedPointNumber.ZERO)
        );
      }

      if (
        type === PriceProviderType.ORACLE &&
        this.priceProviders[PriceProviderType.ORACLE]
      ) {
        return (
          this.priceProviders[PriceProviderType.ORACLE]?.subscribe(token) ||
          of(FixedPointNumber.ZERO)
        );
      }
