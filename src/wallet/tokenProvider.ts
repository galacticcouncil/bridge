...
} from "@acala-network/sdk/wallet/token-provider/type";
import { Storage } from "@acala-network/sdk/utils/storage";
import { createTokenList } from "@acala-network/sdk/wallet/token-provider/create-token-list";
...
export const createStorages = (api: AnyApi) => {
  return {
    ...
    tradingPairs: () =>
      Storage.create<
        [StorageKey<[AcalaPrimitivesTradingPair]>, ModuleDexTradingPairStatus][]
      >({
        api: api,
...
export class AcalaTokenProvider implements TokenProvider {
  ...
  private initTokens() {
    ...
    const foreignAssetLocations$ =
      this.storages.foreignAssetLocations().observable;
      ...
      map(({ tradingPairs, assetMetadatas, foreignAssetLocations }) => {
        return createTokenList(
          ...
          foreignAssetLocations,
          {
            ...
            insertLCDOT:
              this.chainType === ChainType.ACALA ||
              this.chainType === ChainType.MANDALA,
          }
        );
      })
    );
  ...
  public subscribeToken(token: MaybeCurrency): Observable<Token> {
    ...
    return this.tokens$.pipe(
      filter((data) => !!data),
      map((tokens) => {
        const result = Object.values(tokens).find((item) =>
          this.isTokenEqual(name, item)
        );
  ...
  public getToken(token: MaybeCurrency): Token {
    ...
    const result = Object.values(this.tokens$.getValue() || {})
      .reverse()
  ...
  public subscribeTokens(
    types?: TokenType | TokenType[]
  ): Observable<TokenRecord> {
    return this.tokens$.pipe(
      map((data) => {
